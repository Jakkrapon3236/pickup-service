...
export default function Bottombar({ onIconClick }: { onIconClick: (value: number) => void }) {
  ...
  const handleLeave = (setIcon: (path: string) => void, defaultPath: string) => () => setIcon(defaultPath);
  const handleClick = (setIcon: (path: string) => void, clickPath: string, value: number) => () => {
